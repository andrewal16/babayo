...
const Calendar = ({ onDateSelect, selectedDate }) => {
  ...
  const scheduleData = {
    "2024-01-19": [
      { title: "Introduction to IT-Driven Business Growth", time: "07:00 - 09:00" },
      { title: "Business Analytics and Decision Making", time: "13:00 - 14:30" },
    ...
    <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {Array.from({ length: daysInMonth }).map((_, index) => {
          const currentDateString = `${currentDate.getFullYear()}-${String(currentDate.getMonth() + 1).padStart(2, '0')}-${String(index + 1).padStart(2, '0')}`;
          ...
          return (
            <div
              key={index + 1}
              onClick={() => {
                const newDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), index + 1);
                onDateSelect(newDate);
              }}
              className={`text-center p-2 cursor-pointer rounded transition-colors
                ${isSelected ? 'bg-[#001d5b] text-white' : hasSchedule ? 'bg-blue-100 hover:bg-blue-200' : 'hover:bg-gray-100'}
                ${hasSchedule ? 'font-semibold' : ''}
              `}
            >
              {index + 1}
              {hasSchedule && <div className="w-1 h-1 bg-[#001d5b] rounded-full mx-auto mt-1"></div>}
            </div>
          );
        })}
      </div>
      ...
      <main className="ml-64 pt-16 p-8 bg-[#f9f9f0] ">
        <div className="flex">
          ...
          <div className="w-2/3 mt-8">
            <h1 className="text-3xl font-bold text-[#001d5b] mb-8">Your Schedule</h1>
            ...
            <div className="space-y-4">
              {(() => {
                ...
                return daySchedule.map((schedule, index) => (
                  <div key={index} className="bg-[#d3b895] rounded-xl p-6 flex justify-between items-center">
                    <div>
                      <h3 className="text-xl font-semibold text-[#001d5b]">{schedule.title}</h3>
